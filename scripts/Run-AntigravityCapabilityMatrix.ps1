...
if (-not $smokeResult.usable) {
    throw "Antigravity Bridge is not ready (status=$($smokeResult.status), transport=$($smokeResult.transport), delivery=$($smokeResult.delivery_state)): $($smokeResult.error)"
}
